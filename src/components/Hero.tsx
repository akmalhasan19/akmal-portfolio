...
export default function Hero() {
  ...
  return (
    <div className={`${caveat.variable} h-screen w-full bg-[#101010] relative`}>
      ...
      {/* Floating "Close Book" button for Book 1 when user is past page 5 */}
      {cameraPhase === 'focused' && spotlightBook === 'book1' && showBackButton && currentBook1Page > 5 && (
        <button
          onClick={handleBackToOverview}
          className="absolute bottom-8 left-1/2 -translate-x-1/2 z-30 flex items-center gap-2 px-5 py-2.5 rounded-lg border border-[#c7a575]/50 bg-[#1a1410]/80 backdrop-blur-sm text-[#f2e3c7] shadow-[0_4px_24px_rgba(0,0,0,0.5)] transition-all duration-300 hover:bg-[#2d2218] hover:border-[#d2b183] hover:shadow-[0_4px_32px_rgba(199,165,117,0.25)] active:scale-95 group"
        ...
        >
          <svg
            ...
            className="w-5 h-5 text-[#d5bb94] group-hover:text-[#f2e3c7] transition-colors"
          >
            <path d="M4 19.5v-15A2.5 2.5 0 0 1 6.5 2H20v20H6.5a2.5 2.5 0 0 1 0-5H20" />
            <path d="M12 13l-3-3 3-3" />
          </svg>
          <span className="text-lg font-bold tracking-wide">
            {translatedText.closeBookButton}
          </span>
        </button>
      )}
